...
def uLSIF(x_de,x_nu,x_re=[],sigma_list=[],lambda_list=[],b=100,fold=0):
  ...
  if len(sigma_list)==1 && len(lambda_list)==1:
    ...
    for sigma,sigma_index in enumerate(sigma_list):
      ...
      for lamb,lambda_index in enumerate(lambda_list):
        if fold==0:
          C=H+lamb*(n_de-1)/n_de*numpy.eye(b) #eye is identity matrix
          invC=linalg.inv(C)
          beta=invC*h
          invCK_de=invC*K_de2
          tmp=n_de*numpy.ones(1,n_min)-numpy.sum(K_de*invCK_de,1)
    ...
    [score,sigma_chosen_insex]=numpy.min(score_cv_tmp)
  ...
  alphat=mylinsolve(K_de*K_de.conj().tranpose()/n_de+lambda_chosen*numpy.eye(b),numpy.mean(K_nu,2))
